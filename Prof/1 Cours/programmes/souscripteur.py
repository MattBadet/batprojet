...
# avec un seul abonnement vers un sujet
...
def sub_cb(sujet, msg):
    ...
    print("sujet :",sujet, "     message reçu :", msg)# à chaque publication du sujet on affiche le sujet et le message

def connexion_et_abonnement():
    global client_id, mqtt_server, sujet_abonnement_1 # déclaration de l'utilisation de variables globales
...
while True:
    try:        
        client.check_msg() # méthode qui vérifie si un message et disponnible sur le serveur MQTT pour notre client MQTT
