# Programme permettant de publier des messages vers un serveur MQTT
...
mqtt_server = '192.168.50.1'  # adresse IP de notre serveur MQTT (wifi raspberry)
...
sujet_publication_1 = b'temperature'  # déclaration de l'intitulé du sujet 1 : b' indique une chaine de caractères sous forme d'octets
sujet_publication_2 = b'humidite'     # déclaration de l'intitulé du sujet 2 : b' indique une chaine de caractères sous forme d'octets
...
while True:           # La boucle infinie : programme principal
    try:        
        if (time.time() - dernier_message) > interval_entre_message:# si on a dépasser une durée définie par message_interval
            ...
            sensor.measure()            # lecture du capteur
            temp = sensor.temperature() # extraction de la valeur de la température
            hum = sensor.humidity()     # extraction de la valeur de l'humidité
            
            msg = str(temp)             # on transforme un float en str
            ...
            client.publish(sujet_publication_1, msg)# le client MQTT publie son message à propos du sujet "temperature"
                                        # cette publication est envoyée au serveur MQTT c'est à dire au raspberry           
            msg = str(hum)              # comme précédemment mais pour l'humidité
            msg = msg.encode('utf-8')   
            print(type(msg),"message = ",msg)
            client.publish(sujet_publication_2, msg)
